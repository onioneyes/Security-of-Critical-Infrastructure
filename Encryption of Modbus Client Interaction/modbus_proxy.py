...
class Connection:
    ...
    async def _write(self, data):
        if type(self).__name__ =='Client':
            self.log.info("Data to be sent is %r", data)
            data = self.fernet.encrypt(data)
            self.log.info("sending encrypted %r", data)
        else:
            self.log.info("sending %r", data)
    ...
    async def _read(self):
        ...
        if type(self).__name__ =='Client':
            encData = await self.reader.read(1024)
            self.log.info("Encrypted data read is %r", encData)
            reply = self.fernet.decrypt(encData)
        else:	
            header = await self.reader.readexactly(6)
            size = int.from_bytes(header[4:], "big")
            reply = header + await self.reader.readexactly(size)
        self.log.info("received %r", reply)
        return reply
...
class Client(Connection):
    def __init__(self, reader, writer, encryptionKey=None):
        ...
        #loop.run_until_complete(self.keyExchange())
        #self.log.info("Key received is %r " , clientKey)
        #clientKey = rsa.decrypt(clientKey, privateKey).decode()
        self.log.info("Client key is " + str(clientKey))
        self.fernet = Fernet(clientKey)
        self.log.info("new client key = " + str(clientKey))

    def keyExchange(self):
        self.log.info(publicKey.save_pkcs1())
        self.writer.write(publicKey.save_pkcs1())
        #task = asyncio.create_task(getKey())
        #done, pending = await asyncio.wait({task})
        time.sleep(5)
        key= self.reader.read(1024)
        self.log.info(dir(key))
        #if task in pending:
        #    #key = await self.reader.read(1024)
        #    self.log.info(key.send(None))
        #    self.log.info("Key received is %r " , key)
        #    return rsa.decrypt(key, privateKey).decode()
        '''
...
class ModBus(Connection):
    ...
    async def _write_read(self, data):
        self.log.info("Called with " +str(data))
        await self._write(data)
        return await self._read()

    async def handle_client(self, reader, writer):
        async with Client(reader, writer) as client:
            #self.log.info("Exchanging the public key of proxy with client")
            #reply = await self.write_read(request)
            #if not reply:
            #    self.log.info("Called break")
            #    break
			
            self.log.info(publicKey.save_pkcs1())
            client.writer.write(publicKey.save_pkcs1())
            
            #time.sleep(5)
            key= await client.reader.read(1024)
            #self.log.info(dir(key))
            #clientKey = self.keyExchange()
            encryptionKey = rsa.decrypt(key, privateKey)
            self.log.info("Client key is " + str(encryptionKey))
            client.fernet = Fernet(encryptionKey)
			
            while True:
                
                request = await client.read()
                self.log.info("read" +str(request))
                if not request:
                    break
                reply = await self.write_read(request)
                self.log.info("write_read" +str(reply))
                if not reply:
                    break
                result = await client.write(reply)
                self.log.info("write" + str(result))
                if not result:
                    break
