...
def send_message(adu, sock, fernet):
    ...
    encryptedADU = fernet.encrypt(adu)
    print("*"*20 +"Transmission from client to the proxy server"+"*"*20 )
    print(" Command in Plaintext " +str(adu))
    print(" Command  in Encrypted format" +str(encryptedADU))
    sock.sendall(encryptedADU)
    data = sock.recv(1024)
    decryptedData  = fernet.decrypt(data)
    print("*"*20 + "Reception from server to the client"+"*"*20 )
    print("Encryption of the response " +str(data))
    print("Decryption of the response " +str(decryptedData))
    
       
    # Check exception ADU (which is shorter than all other responses) first.
    exception_adu_size = 9
    response_error_adu = recv_exactly(decryptedData, exception_adu_size)
    ...
    response_remainder = recv_exactly(
        decryptedData, expected_response_size - exception_adu_size, exception_adu_size)
